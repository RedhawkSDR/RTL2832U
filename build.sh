...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.RTL2832U.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.RTL2832U-2.0.1
        tar czf ${tmpdir}/rh.RTL2832U-2.0.1.tar.gz --exclude=".svn" -C ${tmpdir} rh.RTL2832U-2.0.1
        rpmbuild -ta ${tmpdir}/rh.RTL2832U-2.0.1.tar.gz
        rm -rf $tmpdir
    else
